...
int main(int argc, char **argv) {
  ...
  int file_location = 0;
  opt_ind = parseFlags(argc, argv, &option, &value, patterns);
  ...
  if (argc >= 3 && option.errorflag != 1 && opt_ind != argc) {
    ...
    while (file_location < argc) {
      ...
      flagsRealisation(value, option, patterns);
      file_location++;
    }
  }
  return 0;
...
int parseFlags(int argc, char **argv, flags *flag, grep_values *value,
  ...
  while ((opt = getopt_long(argc, argv, "e:ivclnhsf:o", NULL, NULL)) != -1) {
    switch (opt) {
      case 'e':
        //цикл идет до того момент пока не найдет пустую ячейку
  ...
  if (access(value.path, F_OK) == 0) {
    ...
    while ((fgets(string, SIZE, filename)) && (stop == 0)) {
      ...
      for (int i = 0; i < value.count_pattern; i++) {
        ...
        if (flag.oflag == 1) flag_o(string, preg);
        regfree(&preg);
      }
    }
    if (flag.cflag == 1) {
      flag_cl(value, flag, count_matched_lines);
    }
    fclose(filename);
